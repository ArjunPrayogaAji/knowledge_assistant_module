...
- **Qdrant** stores: `document_chunks` (3072-dim vectors + payload metadata)
...
| Embeddings | gemini-embedding-001 | **3072 dimensions** — via `google-genai` SDK |
...
- Use google-genai SDK for Gemini (embeddings: gemini-embedding-001 at 3072 dimensions, LLM: gemini-2.5-flash)
...
1. POST /ingest
   ...
   - On startup or before first upsert, ensure Qdrant collection 'document_chunks' exists — create it if not (vectors size=3072, distance=Cosine, with sparse vectors enabled for hybrid search).
...
| Vector dimension = 3072 (matches gemini-embedding-001 output) | MEDIUM |
